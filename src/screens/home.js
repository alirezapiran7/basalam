...
import { generalGet, incrementItemCart } from '../redux/actions'
...
const home = ({ navigation }) => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: color.grey100 }}>
            <View style={{ width: '100%', backgroundColor: color.white, height: 60, marginTop: insets.top, alignItems: 'center', flexDirection: 'row', paddingHorizontal: 16 }}>
                <TouchableOpacity>
                    <IconX name={'handbag'} size={25} origin={ICON_TYPE.SimpleLineIcons} />
                    <Text style={{ minWidth: 15, minHeight: 15, position: 'absolute', top: 0, start: 0, backgroundColor: color.tint, borderRadius: 8, overflow: "hidden", color: color.white, fontSize: 12, textAlign: 'center' }}>1</Text>
                </TouchableOpacity>
            ...
            <FlatList
                ...
                numColumns={2}
                renderItem={({ item }) => (
                    productItem({ item: item, onPress: () => { incrementItemCart(item) } }))
                } />
