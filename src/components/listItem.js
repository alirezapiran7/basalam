        ...
        <TouchableOpacity style={{ width: metrics.screenWidth / 2 - 16, marginVertical: 4, marginHorizontal: 4, borderRadius: 8, overflow: "hidden", backgroundColor: color.white }}
            ...
            <View style={{ height: metrics.screenWidth / 2, }}>
                ...
                <View style={{ minWidth: 70, position: 'absolute', end: 0, bottom: 0, backgroundColor: color.transparentWhite, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', padding: 4 }}>
                    <Text style={{fontSize:12}}>{`(${item.rating.count}) ${item.rating.rating}`}</Text>
            ...
            <View style={{ width: '100%', flexDirection: 'column', padding: 8 }}>
                <Text style={{ width: '100%', textAlign: 'right', fontSize: 12 }} numberOfLines={2}>{item.name}</Text>
                <Text style={{ width: '100%', textAlign: 'right', color: color.gray, fontSize: 10, height: 50 }} numberOfLines={3}>{item.vendor.name}</Text>

                <View style={{ flexDirection: 'row', }}>
                    <Text style={{ flex: 1, textAlign: 'left', fontSize: 12, }}>{currencyFormat(item.price)}</Text>
                    <Text style={{ textAlign: 'right', fontSize: 12, color: color.gray, }}>{weightFormat(item.weight)}</Text>
                </View>
            </View>
        </TouchableOpacity>
